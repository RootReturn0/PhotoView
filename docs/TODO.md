...
- [x] 产品 subagent：把 `docs/plan.md` 全部功能整理为可验收清单。
- [x] 产品 subagent：明确每个功能的完成标准、依赖关系和实现顺序。
- [x] 产品 subagent：确认阶段划分只控制实现顺序，不裁剪任何 `docs/plan.md` 功能。
...
- [x] 已经有单独的标签管理界面了，'全部'界面的标签管理可以去掉
- [x] 标签管理界面添加、编辑标签的弹窗与主题风格不搭，且编辑标签、删除标签的操作过于复杂，需要简化操作
- [x] 标签的设置过于复杂，可以通过下拉菜单(多选)的方式进行设置
- [x] 导入大文件夹(有多层子文件夹，对应多个合集)，软件会出现未响应的情况，可以优化一下性能或者限制一下速度，可以慢，但不能未响应
- [x] 导入的进度显示可以优化一下，当批量导入(含多个子文件的大文件夹时)，可以显示总的合集个数以及当前已完成的个数，有进度条显示，更为直观
- [x] 查看集合图片的时候，当图片的高度过长时，会出现图片越界超过容器的情况，需要优化
- [x] 查看集合里的图片时可以选择网格视图，当使用网格视图时，只展示缩略图和名字，方便浏览集合图片
- [x] 设置标签时，在下拉菜单点击标签项整个软件界面会白屏，需要修复
- [x] 在列表视图下，查看集合图片时，当某个图片height > width时，会导致图片显示越界，需要修复
- [x] 当查看图片详情时，如果图片的height > width，图片显示位置会偏下，且显示不全，无法滚轮移动视角，需要修复
- [x] 加载合集时可以懒加载，避免一次性加载太多的合集导致界面卡顿
- [x] 筛选功能有问题，无法应用搜索，需要修复或者去掉该功能
- [x] Windows/macOS 点击导入按钮或导入大量文件时界面无响应
- [x] 软件支持中英文切换，并覆盖主界面、设置、搜索、标签、提示和无障碍文案
- [x] 设置页支持修改数据库存储路径，迁移后立即重连，并将旧数据库移动到新位置 `.bak`
- [x] 设置页数据管理布局优化，改为上下两段式并强化数据库路径操作层级
- [x] Windows 合集、图片和查看器路径展示隐藏 `\\?\`/`\\?\UNC\` 前缀，内部路径继续保留原始值
- [x] 导入完成后自动隐藏导入进度条
- [x] 图片详情批量操作扩展：支持选择当前筛选、清空选择、批量添加/移除/替换标签、批量收藏/取消收藏、复制多图片路径，并保留批量移动、复制、评分、删除
