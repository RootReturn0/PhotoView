...
- 已新增 `.github/workflows/cloudflare-pages.yml`，配置 push 到 `main` 且 landing 相关文件变化时自动部署 `landing/` 到 Cloudflare Pages；workflow 会先安装 pnpm 并检查 Cloudflare secrets，同时 CI 监听分支从 `master` 改为 `main`。
- 已将新的网页 favicon 同步到落地页自包含资源 `landing/assets/icon-128x128.png`，landing 三个页面继续引用该资源。
