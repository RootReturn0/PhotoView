...
- [x] 文件监听 subagent：监听已导入合集目录变化。
- [x] 文件监听 subagent：自动同步新增、删除、重命名的图片和文件夹。
- [x] 设置/数据 subagent：实现主题、语言、快捷键、缩略图大小设置。
- [x] 设置/数据 subagent：实现数据库备份、恢复、重建索引和数据导出。
...
- [x] 侧边栏，收藏、最近、标签按钮点击无反应
- [x] 已导入集合无法删除
- [x] 缩略图设置无效果，封面还是不显示缩略图
- [x] webp格式的动图无法正常播放，只能看静态图
- [x] 导入某个大文件夹时(有多个子文件夹，对应多个合集)，会卡死
- [x] 整体样式不够美观现代化，可以使用shadCN + tailwindCSS美化，白色简约风主题
- [x] 设置按钮放到侧边栏，设置界面独立一个页面
