...
- [ ] 数据库 subagent：实现 schema migration 和基础 CRUD。
...
- [ ] 图片解码 subagent：处理大图加载、解码失败和占位图状态。
- [ ] 缩略图 subagent：实现缩略图生成任务队列。
- [ ] 缩略图 subagent：实现磁盘缓存、缓存命中和失败重试。
- [ ] 缩略图 subagent：实现缓存大小统计和清理接口。
- [ ] 前端 UI subagent：实现主布局、菜单栏、工具栏、侧边栏和主内容区。
- [ ] 前端 UI subagent：实现合集网格、合集列表、排序和基础筛选。
- [ ] 前端 UI subagent：实现合集详情页和缩略图虚拟滚动。
- [ ] 查看器 subagent：实现图片打开、上一张/下一张、键盘快捷键。
- [ ] 查看器 subagent：实现适应窗口、实际大小、自由缩放和旋转。
- [ ] 查看器 subagent：实现全屏模式、幻灯片和图片信息面板。

## 阶段 3：管理能力

- [ ] 合集管理 subagent：实现合集编辑，包括名称、描述、评分、封面。
- [ ] 合集管理 subagent：实现合集收藏、最近查看、查看次数更新。
- [ ] 合集管理 subagent：实现合集删除记录，并预留是否删除物理文件夹的确认流程。
- [ ] 图片管理 subagent：实现单图打开、重命名、移动、复制、删除。
- [ ] 图片管理 subagent：实现批量移动、批量复制、批量删除、批量评分。
- [ ] 图片管理 subagent：实现右键菜单和拖拽操作。
- [ ] 标签系统 subagent：实现标签创建、编辑、删除、重命名。
- [ ] 标签系统 subagent：实现标签颜色和合集/图片多标签关联。
- [ ] 标签系统 subagent：实现按标签筛选合集和图片。

## 阶段 4：高级功能

- [ ] 搜索 subagent：实现全局搜索合集名称、图片文件名和标签。
- [ ] 搜索 subagent：实现高级组合搜索，包括格式、分辨率、大小、标签、评分、日期、收藏状态。
- [ ] 搜索 subagent：实现搜索结果分类展示和点击跳转定位。
- [ ] 重复检测 subagent：实现 SHA256 完全重复检测。
- [ ] 重复检测 subagent：实现 pHash 视觉相似检测。
- [ ] 重复检测 subagent：实现重复/相似图片分组、删除建议和批量处理。
